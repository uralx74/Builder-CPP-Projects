...
typedef struct {
    ...
    //std::vector <PARAMLISTITEM> variables;   // Список возможных значений
    std::vector <PARAMLISTITEM> listitem;   // Список значений (для list и variables)
...
typedef struct {
    ...
    std::vector<PARAMRECORD> UserParams;    // Задаваемые параметры к запро
...
typedef struct {    // Структура, для хранения структуры вкладок
    AnsiString name;
    std::vector<QUERYITEM*> queryitem;
} TABITEM;
...
private:	// User declarations
    ...
    void ParseUserParamsStr(AnsiString ParamStr, QUERYITEM* queryitem);  // Разбор xml в std::vector<PARAMRECORD>
    void ParseExportParamsStr(AnsiString ParamStr, QUERYITEM* queryitem);  // Разбор xml в std::vector<DBASEFIELD>
    ...
    int __fastcall DataSetToQueryList(TOraQuery* oraquery, std::vector<QUERYITEM>& query_list, std::vector<TABITEM>& tab_list);
    ...
    QUERYITEM* CurrentQueryItem; 	    // Текущий выбранный запрос
