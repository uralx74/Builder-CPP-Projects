...
//---------------------------------------------------------------------------
// Загружает вектор
int __fastcall TForm1::DataSetToQueryList(TOraQuery* oraquery, std::vector<QUERYITEM>& query_list, std::vector<TABITEM>& tab_list)
{
...
void TForm1::ParseUserParamsStr(AnsiString ParamStr, QUERYITEM* queryitem)
{
    if (ParamStr == "")
        return;
    ...
    std::vector<PARAMRECORD>* ListParams = &queryitem->UserParams;
    ...
    while (!node.IsEmpty())
    {
        ...
        param.name = LowerCase(msxml.GetAttributeValue(node, "name"));
...
void TForm1::ParseExportParamsStr(AnsiString ParseStr, QUERYITEM* queryitem)
{
    if (ParseStr == "") {
...
void TForm1::InitEnvVariables()
{
    ...
    if (Username.UpperCase() == "ADMIN") {
        ...
        OraQuery->SQL->Add("select id, uch from raion"
...
void TForm1::FillParametersLV()
{
    ...
    QUERYITEM* qi = TabList[TabControl1->TabIndex].queryitem[ListBox1->ItemIndex];
	...
	for (unsigned int i = 0; i < CurrentQueryItem->UserParams.size(); i++) {
        PARAMRECORD *record = &CurrentQueryItem->UserParams[i];
...
//AnsiString TForm1::ReplaceQueryVariables(std::vector<PARAMRECORD>* ListParams)
...
void TForm1::FillFieldsLB()
{
    ...
    TABITEM* TabItem = &TabList[TabControl1->TabIndex];
...
void __fastcall TForm1::DinamicControlExit(TObject *Sender)
{
    if (Sender != NULL)
    {
        ...
        PARAMRECORD *param = &CurrentQueryItem->UserParams[Control->Tag];
        if (param->type == "list")
        {
            ...
            for (int i = 0; i < n; i++) {
                PARAMRECORD *p = &CurrentQueryItem->UserParams[i];
                if (p->parent != "") {
...
void __fastcall TForm1::OnEditParam()
{
    ...
    for (paramitem_index = 0; paramitem_index < CurrentQueryItem->UserParams.size(); paramitem_index++) {
        PARAMRECORD *param = &CurrentQueryItem->UserParams[paramitem_index];
    ...
    if (param->type == "date")
    {
        ...
        {
            PARAMLISTITEM item = param->listitem[i];
